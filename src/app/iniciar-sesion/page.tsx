...
export default function LoginPage() {
  ...
  const { authUser, profile, loading, signIn, isAdminMasterSession } = useAuth();
  ...
  useEffect(() => {
    if (!loading && authUser) {
      if (isAdminMasterSession && profile?.role === "admin") {
      ...
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-md items-center justify-center md:min-h-[calc(100vh-4rem)]">
        <section className="w-full rounded-2xl border border-slate-100 bg-white p-6 shadow-xl shadow-slate-200/50">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-slate-600">Ingresa tu PIN de 4 dígitos.</p>
            <p className="mt-1 text-xs text-slate-500">
              Si eres administrador, también puedes ingresar con el PIN maestro de 4 dígitos.
            </p>
          </div>
