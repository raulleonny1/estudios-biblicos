...
type FaithTransformCourseViewProps = {
  study: Study;
};

export function FaithTransformCourseView({ study }: FaithTransformCourseViewProps) {
  ...
  const totalStudies = faithTransformLessons.length;
  ...
  const unlockedLessonIds = useMemo(
    () =>
      getUnlockedLessonIds({
        lessonIds: faithTransformLessons.map((item) => item.id),
        submissions,
      }),
    [submissions]
  );
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-50 via-indigo-50 to-sky-50 font-sans">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-2">
          {faithTransformLessons.map((lesson) => {
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <Link
                    href={isUnlocked ? `/fe/lecciones/${lesson.id}` : "#"}
                  ...
                  <Link
                    href={isUnlocked ? `/fe/lecciones/${lesson.id}/reforzar` : "#"}
                    aria-disabled={!isUnlocked}
                    className={`inline-flex rounded-md px-4 py-2 text-sm font-medium text-white transition ${
                      isUnlocked
                        ? "bg-zinc-900 hover:bg-indigo-700"
