...
type StewardshipIntermediateCourseViewProps = {
  study: Study;
};

export function StewardshipIntermediateCourseView({ study }: StewardshipIntermediateCourseViewProps) {
  ...
  const totalStudies = stewardshipIntermediateLessons.length;
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-50 via-indigo-50 to-sky-50 font-sans">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700">
                <BookOpen size={12} />
                Curso de mayordomía intermedia
              </p>
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-2">
          {stewardshipIntermediateLessons.map((lesson) => {
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link
                      href={isUnlocked ? `/mayordomia-intermedia/lecciones/${lesson.id}` : "#"}
                    ...
                    <Link
                      href={isUnlocked ? `/mayordomia-intermedia/lecciones/${lesson.id}/reforzar` : "#"}
                      aria-disabled={!isUnlocked}
                      className={`inline-flex rounded-md px-4 py-2 text-sm font-medium text-white transition ${
                        isUnlocked
                          ? "bg-zinc-900 hover:bg-indigo-700"
